...
export async function fetchProjects(): Promise<Project[]> {
  ...
  try {
    ...
    // Debug: log first record's field names to verify mapping
    if (allRecords.length > 0) {
      console.log("🔍 Airtable field names:", Object.keys(allRecords[0].fields));
      console.log("🔍 First record sample:", JSON.stringify(allRecords[0].fields, null, 2));
    }
