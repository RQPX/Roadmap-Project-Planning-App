...
import { Plus, Info, ChevronLeft, ChevronRight, TrendingUp, TrendingDown, Minus } from "lucide-react";
...
export default function ProjectsKanban() {
  ...
  const projectsFilteredByDirection = filterDirection === "all" ? projects : projects.filter((p) => p.department === filterDirection);
  ...
  const paginatedProjects = filteredProjects.slice(currentPage * CARDS_PER_PAGE, (currentPage + 1) * CARDS_PER_PAGE);
  ...
  return (
    <div style={{ padding: "16px", maxWidth: "1600px", margin: "0 auto" }}>
      ...
      <div className="hide-scrollbar" style={{ overflowX: "auto", paddingBottom: "8px", marginBottom: "12px", scrollbarWidth: "none" } as React.CSSProperties}>
        <div style={{ display: "flex", gap: "8px", minWidth: "max-content", alignItems: "center" }}>
          ...
          <button onClick={() => setFilterDirection("all")} style={{ padding: "4px 12px", borderRadius: "9999px", fontSize: "12px", fontWeight: 500, cursor: "pointer", border: "none", backgroundColor: filterDirection === "all" ? "#111827" : "#f3f4f6", color: filterDirection === "all" ? "white" : "#374151" }}>
            Toutes ({projects.length})
          </button>
          {allDirections.map((dir) => (
            <button key={dir} onClick={() => setFilterDirection(dir)}
              style={{ padding: "4px 12px", borderRadius: "9999px", fontSize: "12px", fontWeight: 500, cursor: "pointer", border: "none", backgroundColor: filterDirection === dir ? "#111827" : "#f3f4f6", color: filterDirection === dir ? "white" : "#374151" }}>
              {dir} ({projects.filter((p) => p.department === dir).length})
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Onglets statut */}
      <div className="hide-scrollbar" style={{ overflowX: "auto", paddingBottom: "8px", marginBottom: "20px", scrollbarWidth: "none", msOverflowStyle: "none" } as React.CSSProperties}>
        <div style={{ display: "flex", gap: "8px", minWidth: "max-content" }}>
          {allStatuses.map((status, index) => {
            ...
            return (
              <button key={status} onClick={() => setActiveStatusIndex(index)}
                style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 14px", borderRadius: "9999px", fontSize: "13px", fontWeight: 500, whiteSpace: "nowrap", cursor: "pointer", border: "none", backgroundColor: isActive ? "#2563eb" : "#f3f4f6", color: isActive ? "white" : "#374151" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
        ...
        {filterDirection !== "all" && <span style={{ fontSize: "12px", color: "#9ca3af", marginLeft: "4px" }}>— {filterDirection}</span>}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        {paginatedProjects.length === 0 ? (
          ...
          paginatedProjects.map((project) => {
            const currentPct = formatProgressValue(project.progress);
            const prevPct = project.previousMonthProgress != null ? formatProgressValue(project.previousMonthProgress) : null;
            const diff = prevPct != null ? currentPct - prevPct : null;
              ...
              <div key={project.id} onClick={() => handleEditProject(project)}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "6px", flex: 1 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                      ...
                      <Badge style={{ backgroundColor: priorityColors[project.priority], color: "white" }} className="text-xs">{project.priority}</Badge>
                      {isProjectDelayed(project.endDate, project.status) && <Badge className="bg-red-600 text-white text-xs">RETARD</Badge>}
                    </div>
                  </div>

                  {/* Taux mois précédent */}
                  {prevPct != null && (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "2px", marginLeft: "12px", flexShrink: 0 }}>
                      <span style={{ fontSize: "10px", color: "#9ca3af", whiteSpace: "nowrap" }}>Mois précédent</span>
                      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                        <span style={{ fontSize: "13px", fontWeight: 600, color: "#374151" }}>{prevPct}%</span>
                        {diff !== null && diff > 0 && <TrendingUp style={{ width: 14, height: 14, color: "#16a34a" }} />}
                        {diff !== null && diff < 0 && <TrendingDown style={{ width: 14, height: 14, color: "#dc2626" }} />}
                        {diff !== null && diff === 0 && <Minus style={{ width: 14, height: 14, color: "#6b7280" }} />}
                        {diff !== null && (
                          <span style={{ fontSize: "11px", color: diff > 0 ? "#16a34a" : diff < 0 ? "#dc2626" : "#6b7280", fontWeight: 500 }}>
                            {diff > 0 ? `+${diff}` : diff}%
                          </span>
                        )}
                      </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#4b5563" }}>
                    ...
                    <span style={{ fontWeight: 500 }}>{currentPct}%</span>
                  </div>
